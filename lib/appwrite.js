...
//     const user = await getCurrentUser(); 
//     if (!user) return;
//     const allTokens = user.expoPushToken
//     const newTokens = allTokens.split(",,,,")
//     const filteredTokens = newTokens.filter(tok => tok !== token)
//     await updateUser(user.$id, { expoPushToken: filteredTokens.join(",,,,")});
// }
...
export async function deleteExpoPushToken(token) {
    ...
    // Only update if the token actually exists
...
//             .then(async token => token && await saveExpoPushToken(token));
//         return session;
//     } catch (error) {
//         throw new Error(error);
//     }
...
export async function signOut() {
    try {
        // registerForPushNotificationsAsync()
        //     .then(async token => token && await deleteExpoPushToken(token));
