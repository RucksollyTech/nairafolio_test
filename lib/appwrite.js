...
export async function createUserInvestment(investmentId, value_investment, userId) {
    try {
...
export async function getUserTransactionsWithLimit(userId) {
    try {
        const transactions = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.transactionCollectionId,
            [Query.equal("user", userId), Query.limit(7)]
        );
        return transactions.documents;
    } catch (error) {
        throw new Error(error);
