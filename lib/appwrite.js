...
export async function createUserInvestment(
    ...
    try {
        const getDateNeeded=async(ids)=>{
            ...
            if(result && !result.immediate_start && result.date_to_introduction){
                return UTCDate(result.date_to_introduction).isoDate
            }
            return appwriteDatetime
        }
        const dateNeeded = await getDateNeeded(investmentId)
...
export async function getUserInvestments(userId) {
    try {
        const investments = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.user_investmentCollectionId,
            [Query.equal("user", userId),Query.equal("is_matured", false),Query.equal("is_cancelled", false),Query.equal("sold", false),Query.orderDesc("$createdAt")]
        );
    ...
    try {
        ...
        if(query && category){
            ...
            if(category !== "All"){
                ...
                return [...new Set([...name,...company_name,...company_owner])]
            }
