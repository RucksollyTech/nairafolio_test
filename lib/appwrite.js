    ...
    try {
        const getDateNeeded=async(ids)=>{
            ...
            if(result && result[0] && !result[0].immediate_start && result[0].date_to_introduction){
                const dateNeeded = UTCDate(result[0].date_to_introduction).isoDate
...
export async function getDataDollarInvestment() {
    try {
        const dollar = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.investmentCollectionId,
            [Query.equal("category", "Dollar"),Query.orderDesc("$createdAt")]
        );
        return dollar.documents[0]
    } catch (error) {
        throw new Error(error);
    }
}
