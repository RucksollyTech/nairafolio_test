...
export const handlePaymentSuccess = async(reference,investmentId,type,setUser) =>{
    ...
    if(verify?.status && user){
        ...
        if (investmentId === "Unavailable"){
            ...
            await createUserInvestment(investmentId,(verify?.data?.amount)/100,userId,((verify?.data?.amount)/100)/(investmentExtract?.price_per_unit),investmentExtract?.price_per_unit,investmentExtract?.rio)
            await createTransactions({
                action: "Deposit",
                amount: parseFloat((verify?.data?.amount)/100),
                type: type || "Transfer",
