import { createTransactions, createUserInvestment, getCurrentUser, getInvestment, getUser, getUserInvestmentData, updateOngoingInvestment, updateUser } from "./appwrite"
...
export const handlePaymentSuccessFromSales = async(reference,investmentId,type,setUser) =>{
    ...
    if(verify?.status && user){
        await Promise.all(
            ...
            updateUser(sellerUserId,{wallet_balance: parseFloat(sellingUser?.wallet_balance + ((verify?.data?.amount)/100))}),
            createTransactions({
                action: "Deposit",
                amount: parseFloat((verify?.data?.amount)/100),
                type: type || "Transfer",
