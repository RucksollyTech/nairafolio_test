import { createTransactions, createUserInvestment, getCurrentUser, getInvestment, updateUser } from "./appwrite"
...
export const handlePaymentSuccess = async(reference,investmentId,type,setUser) =>{
    ...
    if(verify?.status && user){
        ...
        if (investmentId === "Unavailable"){
            ...
            await createUserInvestment(investmentId,(verify?.data?.amount)/100,userId)
            await createTransactions({
                action: "Deposit",
                amount: parseFloat((verify?.data?.amount)/100),
                type: type || "Transfer",
